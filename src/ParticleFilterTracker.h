...
{
public:
    explicit ParticleFilterTracker(Point detection,
                                   MatND histogram,
                          ...
                          unsigned int noParticles=200,
                                   ...
                                   double var_m=0.5,
                                   double hc=0.5);
    void assignDetection(Point detection, MatND histogram);
    void update();
    Point getPos();
    State getState();
    MatND histogram;
...
protected:
    ...
    void predict();
    void measurement(Point detection, MatND histogram);
    void systematicResampling(vector<double>& w, vector<int>& indx);
    void initialize(Point detection);
